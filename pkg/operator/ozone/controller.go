...
func (c *Controller) applyService(resource *data.Resource,
	...
	original, err := c.context.Clientset.CoreV1().Services(objectStore.Namespace).Get(resource.Name(), metav1.GetOptions{})
	if err == nil {
		service.ResourceVersion = original.ResourceVersion
		_, err = c.context.Clientset.CoreV1().Services(objectStore.Namespace).Update(&service)
	} else if errors.IsNotFound(err) {
		_, err = c.context.Clientset.CoreV1().Services(objectStore.Namespace).Create(&service)
